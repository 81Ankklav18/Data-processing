...
public class Google extends Logic implements Initializable {
    ...
    public void CrossCorellation(ActionEvent actionEvent) throws IOException {
        ...
        for(int i = 0; i < 300; i++){
            if (i > 99 && i < 200){
                series1.getData().add(new XYChart.Data<Double, Double>((double)i, 100.d));
            }
            if (i > 49 && i < 250){
                series2.getData().add(new XYChart.Data<Double, Double>((double)i, 100.d));
            }

            series1.getData().add(new XYChart.Data<Double, Double>((double)i, 0.d));
            series2.getData().add(new XYChart.Data<Double, Double>((double)i, 0.d));
        }

        chart1.getData().add(CrossCorrelation(series1, series2));
*/
        if (NetflixRB.isSelected()){
            chart1.getData().add(CrossCorrelation(getSeries(csvpath), getSeries("src/netflix5.csv")));
        }
        if (GoogleRB.isSelected()){
            chart1.getData().add(CrossCorrelation(getSeries(csvpath), getSeries("src/google5.csv")));
        }
        if (AppleRB.isSelected()){
            chart1.getData().add(CrossCorrelation(getSeries(csvpath), getSeries("src/apple5.csv")));
        }
        if (MicrosoftRB.isSelected()){
            chart1.getData().add(CrossCorrelation(getSeries(csvpath), getSeries("src/microsoft5.csv")));
        }
        if (ItoRB.isSelected()){
            chart1.getData().add(SelfCorrelation(getSeries(csvpath),
                    ItoProcess(size()
                            , Double.valueOf(itoa.getText())
                            , Double.valueOf(itoc.getText())
                            , Double.valueOf(itob.getText())
